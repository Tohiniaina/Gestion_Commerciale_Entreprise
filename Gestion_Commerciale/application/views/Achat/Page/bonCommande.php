...
<main role="main" class="container" class="main-content" style="margin-bottom: 50px;">
				...
				<?php for ($i=0; $i < count($boncommande); $i++) { ?>
					<div class="card shadow " style="border-top-color: gray; margin-top: 10px" >
						<h3 class="text-center p-4">Bon de Commande</h3>
						<div class="card-body p-5">
							<small class="small text-muted text-uppercase"><?= $lettre->formatterDateEnLettres($boncommande[$i]['dateboncommande'])?></small><br />
							<small class="small text-muted text-uppercase">N° Bon de Commande : <?= $boncommande[$i]['idboncommande']?></small><br /><br>
							<p>
							...
							<p>
								<small class="small text-muted text-uppercase">Fournisseur : <?= $boncommande[$i]['nom']?></small><br />
								<small class="small text-muted text-uppercase">Adresse : <?= $boncommande[$i]['adresse']?></small><br />
								<small class="small text-muted text-uppercase">Contact : <?= $boncommande[$i]['contact']?></small><br />
								<small class="small text-muted text-uppercase">Email : <?= $boncommande[$i]['email']?></small><br />
							</p>
							...
							<table class="table table-borderless table-striped">
								...
								<tbody>
									...
									<tr>
										<th scope="row">1</th>
										<td class="text-right"><?=$boncommande[$i]['detail'][$j]['nomproduit'] ?></td>
										<td class="text-right"><?=$qte=$boncommande[$i]['detail'][$j]['quantite'] ?></td>
										<td class="text-right"><?=$boncommande[$i]['detail'][$j]['unite'] ?></td>
										<td class="text-right"><?=number_format($ht=$boncommande[$i]['detail'][$j]['prixunitaire'], 0, ',', ' ').',00'; $totalHT+=($ht*$qte) ?></td>
										<td class="text-right"><?php 
										if($boncommande[$i]['detail'][$j]['tva'] == 1) {
											echo $tva=(($boncommande[$i]['detail'][$j]['prixunitaire']*20)/100); 
											$totalTVA+=($tva*$qte); 
										} else {
											$tva = 0;
											echo "0,00";
										...
										?></td>
										<td class="text-right"><?=number_format($ttc=$boncommande[$i]['detail'][$j]['prixunitaire']+$tva, 0, ',', ' ').',00' ?></td>
				...
				<?php } ?>
		<a href="<?php echo site_url("ViewAchat/EnvoiBonCommande");?>"><button class="btn btn-primary" style="margin-top: 30px; float">Envoyer finance</button></a>
</main>
